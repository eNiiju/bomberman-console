...
```mermaid
graph TD
    MAIN_THREAD(Client main thread) --> MAIN_THREAD_A[Get main message queue<br />from game code]
    ...
    MAIN_THREAD_E --> MAIN_THREAD_F[Start game end message thread]
    ...
    THREAD_GAME_STATE_MESSAGE_H --> THREAD_GAME_STATE_MESSAGE_I[Initialize display: ncurses window<br />and color pairs]
    ...
    THREAD_GAME_END_MESSAGE_B --> THREAD_GAME_END_MESSAGE_C[Post display semaphore to end<br />display thread then destroy it]
    ...
    THREAD_INPUTS_C -->|Key pressed| THREAD_INPUTS_E[Send corresponding message<br />to client message queue if valid key]
    ...
    THREAD_DISPLAY_B -->|Yes| THREAD_DISPLAY_D[Display informations, map,<br />powerups, bombs & players]
